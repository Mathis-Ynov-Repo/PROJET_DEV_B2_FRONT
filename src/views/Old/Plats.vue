  ...
  <div class="plats">
    ...
    <section id="plats_list" class="secondary">
      <v-container class="secondary">
        <v-row class="mb-6 flex-row" no-gutters v-if="loading">
        ...
        <v-row class="mb-6" no-gutters>
          <v-col cols="12" md="6" lg="4" xl="3" v-for="plat in filteredData" :key="plat.route">
            <v-card class="mx-auto my-12" max-width="374">
              ...
              <v-card-text>
                ...
                <div class="my-4 subtitle-1">$ • {{plat.platType.type}}</div>
...
export default {
  ...
  methods: {
    ...
    async getPlats() {
      await Axios.get("http://localhost:8001/api/plats").then(
        response => (this.plats = response.data)
      );
    },
    async getPlatsTypes() {
      await Axios.get("http://localhost:8001/api/plats-types").then(
